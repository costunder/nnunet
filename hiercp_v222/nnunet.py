...
TRAINER='nnUNetTrainer_250epochs_OnlinePromptGraphV222'
...
def runtime(root):
    ...
    path=trainer/'nnUNetTrainer_OnlinePromptGraphV222.py'
    if path.exists(): raise ValueError('Conflicting pre-existing v2 trainer')
    shutil.copy2(ROOT/'hiercp_v222/nnUNetTrainer_OnlinePromptGraphV222.py',path)
...
def prepare(medical,split_path,output,dataset_id,cfg,identities_path):
    ...
    identities=validate_identities(read_json(identities_path),split)
    ...
    name=f'Dataset{int(dataset_id):03d}_PromptGraphV222'; env=environment(root)
    ...
    payload={'format':PIPELINE_VERSION,'complete':True,'root':str(root),'medical':str(Path(medical).resolve()),
             ...
             'planning_patient_ids':split['outer_train'],'native_fold':0,'identities':identities}
...
def train(native_path,bank_path,*,workers,seed=42):
    if not 0<=seed<2**32:raise ValueError('Seed must be a uint32')
    if seed!=42:raise ValueError('Paired Basic CP80 comparison requires seed42')
    if workers<1:raise ValueError('Supply a positive, measured worker count shared with Basic CP')
    ...
    env.update(ONLINE_CP_BANK=str(Path(bank_path).resolve()),ONLINE_CP_SEED=str(seed),
               COMPARISON_SEED=str(seed),nnUNet_n_proc_DA=str(workers))
    ...
    write_new(root/'training_started.json',{'format':PIPELINE_VERSION,'bank':str(Path(bank_path).resolve()),'bank_sha256':sha(bank_path),
                                          ...
                                          'seed':seed,'workers':workers,'randomness_contract':RNG_FORMAT,
                                          'cp_probability':bank['cp_probability'],
                                          'physical_batch':read_json(native['plans'])['configurations']['3d_fullres']['batch_size']})
