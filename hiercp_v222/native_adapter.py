...
def request_entry(recipient,donor_index):
    address=tuple(json.loads(os.environ['V222_CP_RPC_ADDRESS']))
    with Client(address,authkey=bytes.fromhex(os.environ['V222_CP_RPC_AUTH'])) as connection:
...
class SharedDonorLoader(PairedLoaderMixin,nnUNetDataLoaderOnlineCP):
...
def native_dataloaders(trainer):
    """Common RNG scheduling; frozen v1 paste code and native validation are preserved."""
    return paired_dataloaders(trainer,SharedDonorLoader,dict(
        bank_path=trainer.online_bank_path,policy=trainer.online_policy,online_seed=trainer.online_seed))
