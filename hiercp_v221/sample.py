...
def _balanced_indices(
    ...
    *,
    ...
    radial = np.clip(shell_ids.astype(np.int64), 0, max(0, int(radial_bins)-1))
    az = np.clip(
...
def _select_context(
    ...
    shell_ids = _numpy(node["shell_id"], dtype=np.int64)
...
def build_local_view(
    ...
    *,
    ...
    if set(all_nodes)!=set(LOCAL_NODE_TYPES):
        raise ValueError('Retired/unknown canonical node types; rebuild surface/context-only cache')
    ...
    selected: dict[str, np.ndarray] = {
        "tumor_surface": np.arange(
            int(source_nodes["tumor_surface"]["grid"].shape[0]), dtype=np.int64
        ),
        "source_liver_surface": np.arange(
            int(source_nodes["source_liver_surface"]["grid"].shape[0]), dtype=np.int64
        ),
        "target_liver_surface": np.arange(
            int(target_nodes["target_liver_surface"]["grid"].shape[0]), dtype=np.int64
        ),
    }
    ...
    for node_type in LOCAL_NODE_TYPES:
        ...
        graph[node_type].num_nodes = len(ids)
    ...
    for edge_type in LOCAL_EDGE_TYPES:
        ...
        edge_index = _induced_edge_index(
            ...
            source_count=int(all_nodes[source_type]["grid"].shape[0]),
            destination_count=int(all_nodes[destination_type]["grid"].shape[0]),
        )
        edge_count = int(edge_index.shape[1])
        edge_counts.append(edge_count)
        graph[edge_type].edge_index = torch.from_numpy(edge_index.astype(np.int64, copy=False))
    ...
    graph.canonical_counts = torch.tensor(
        [[int(all_nodes[node_type]["grid"].shape[0]) for node_type in LOCAL_NODE_TYPES]],
        dtype=torch.int64,
    )
