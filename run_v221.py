"""Independent v2.21 relational T/F/U CLI. Existing run.py and the v1 feedback launcher are unchanged."""
from pathlib import Path
import argparse
import json

def main():
    from hiercp_v221.contracts import load_config,verify_v1,read_json,write_new,validate_split
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--config',default=str(Path(__file__).parent/'config/prompt_graph_v221.json'))
    ...
    if args.command not in ('check','import-split'):
        from hiercp_v221.contracts import require_training_objective
