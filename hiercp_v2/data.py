...
class SourceCollection:
    ...
    def __getitem__(self,index):
        ...
        center=tuple((s.start+s.stop-1)//2 for s in box)
...
def local_record(case,source,center,base,*,prepared=None,depth=None,organ=None):
    ...
    return {'format':PIPELINE_VERSION,'case_id':case.paths.case_id,'component_id':source.component_id,
            'seed':base['seed'],
            'source_patch':torch.from_numpy(built.source_patch).half(),'target_patch':torch.from_numpy(built.target_patch).half(),
...
def prepare(medical,split_path,output,cfg,base):
    ...
    def one(job):
        with volumes.pair(job['recipient'],job['donor_case']) as (target,donor):
            ...
            if job['recipient']!=job['donor_case']:
                from hiercp.local import exact_source_footprint
    ...
    write_new(root/'index.json',{'format':PIPELINE_VERSION,'complete':True,'split':split,'donor_pool':pool,
              ...
              'hard_F_policy':'geometrically invalid placements excluded; normal liver remains U'})
...
class MemoryLocalDataset(Dataset):
    ...
    def __init__(self,records,case_id):
        from hiercp.training_resources import _local_bound
...
class LocalBatch:
    ...
    def to(self,device):
        ...
        self.source_patches=self.source_patches.to(device,non_blocking=True).float()
        self.target_patches=self.target_patches.to(device,non_blocking=True).float()
        self.source_index=self.source_index.to(device,non_blocking=True)
...
def collate(items):
    payload,indices=zip(*items); graphs,sources_,targets=zip(*payload)
    return LocalBatch(Batch.from_data_list(list(graphs)),torch.stack(sources_),torch.stack(targets),
