...
TRAINER='nnUNetTrainer_250epochs_OnlinePromptGraphV2'
...
def runtime(root):
    ...
    path=trainer/'nnUNetTrainer_OnlinePromptGraphV2.py'
    if path.exists(): raise ValueError('Conflicting pre-existing v2 trainer')
    shutil.copy2(ROOT/'hiercp_v2/nnUNetTrainer_OnlinePromptGraphV2.py',path)
...
def prepare(medical,split_path,output,dataset_id,cfg):
    ...
    name=f'Dataset{int(dataset_id):03d}_PromptGraphV2'; env=environment(root)
