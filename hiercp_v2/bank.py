...
from hiercp.local import prepare_local_source
...
from .data import sources,local_record
...
def build(native_path,checkpoint,output):
    ...
    metadata={'format':'hiercp_online_bank_v2','pipeline_version':PIPELINE_VERSION,'complete':True,
        ...
        'tumor_label':2,'liver_label':1,'cp_probability':.5,'intensity_scale_range':[.95,1.05],
...
def validate_catalog(meta):
    from .donors import POLICY,validate_pool
    ...
    if meta['candidate_count']!=128 or meta['cp_probability']!=.5:raise ValueError('CP scale changed')
    return meta
...
class EntryBuilder:
    ...
    def materialize(self,recipient,donor_index):
        ...
        from .donors import select_donor
        ...
        with lock:
            ...
            with self.volumes.pair(recipient,donor_id) as (target,donor):
                ...
                prepared=prepare_local_source(dc,original,full_organ_mask=donor['organ'],organ_depth=donor['depth'],
                    config=graph_config_from_dict(self.base['graph']),rng=np.random.default_rng(self.cfg['seed']),ct_clip=tuple(self.base['ct_clip']))
                ...
                def graph(item):
                    i,center=item
                    record=local_record(raw,source,center,self.base,prepared=prepared,depth=target['depth'],organ=target['organ'])
                    record.update(donor_case_id=donor_id,evidence=-1)
                    return i,record
