...
def validate_resume(saved, checkpoint, *args, **kwargs):
    ...
    if policy is not None and policy.get('runtime_sha256') in (prior_identity(),PUBLISHED_PROCESS_RUNTIME,PUBLISHED_SNAPSHOT_RUNTIME):
...
def main():
    ...
    semantics=dict(feature_coordinates=feature_coordinates,support_task_contract=reviewed.TASK_CONTRACT,
