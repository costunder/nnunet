...
def main():
    cache,output,policy=sys.argv[1:];root=Path(output);root.mkdir(parents=True,exist_ok=False)
    ...
    net=model(cfg,base).cuda();net.local.dense_batch_size=64
    ...
    batches=list(groups(dataset,64,42,0))[:10] # Explicit DEBUG profile, production cohort untouched.
    ...
    try:
        for i in range(len(batches)):
            ...
            row=dict(debug=True,full_training=False,support_scope='six real CT regression observations, not full training memory',
                ...
                allocator_policy=policy,loss=float(loss.detach()))
