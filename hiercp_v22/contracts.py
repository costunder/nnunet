...
def load_config(path=None):
    path = Path(path or ROOT / 'config/prompt_graph_v22.json').resolve()
    cfg = read_json(path)
...
def source_identity():
    paths = sorted(set((ROOT / 'hiercp_v22').glob('*.py')) | set((ROOT/'hiercp').glob('*.py')) |
                   set((ROOT/'custom_trainers').glob('*.py')) |
                   {ROOT/'run_v22.py',ROOT/'config/prompt_graph_v22.json',ROOT/'config/train.json',
                    ROOT/'tools/online_raw_bank_preparation.py'})
    return {p.relative_to(ROOT).as_posix(): sha(p) for p in paths}


def validate_encoder_config(cfg):
    expected={'format':PIPELINE_VERSION,'local_encoder':'ct_only_cnn_connectivity_gnn',
              'feature_contract':'ct_only_cnn_node_features_connectivity_only_r3',
              'descriptor_contract':None,'loss_weights':{'observed_transfer':1.0},'cnn_input_channels':1,
              'cnn_feature_dim':32,'node_auxiliary_feature_dim':0,'edge_attribute_dim':0,
              'implementation_scope':'L0_L1_L2','l2_status':'alignment_and_observed_transfer_restored',
              'training_objective_status':'incomplete_after_statistical_auxiliary_removal',
              'architecture_fixed':False,'freeze_weights':False,
              'topology_contract':'surface_context_no_interior_r5','tumor_interior_nodes':False,
              'l1_contract_status':'unverified_pair_data_unit_and_missing_TFU_edge_conditioning'}
    if any(k not in cfg or cfg[k]!=v for k,v in expected.items()):
        raise ValueError('v2.2 r5 requires CT-only CNN surface/context nodes; L1 semantics remain unverified')
    if cfg.get('label_count')!=16:raise ValueError('Preserve the v2.1 16-label capacity')

def require_training_objective():
    raise RuntimeError('Full training objective is incomplete after statistical auxiliary removal. '
                       'L2 alignment and observed-transfer loss are implemented and connected; '
                       'T/U-only observed loss admits uniform-score collapse. '
                       'L1 also lacks T/F/U-conditioned data-label edges and its paired data unit is unverified. '
                       'Do not run a full experiment with this term alone or invent negatives from U.')
