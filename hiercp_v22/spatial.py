...
The existing dense encoder still receives a fixed ``patch_size쨀`` tensor, but
all topology geometry and semantic nodes are computed in an adaptive native
...
def build_patch_payload(
    *,
    ...
    """Build CT-only CNN input and mask geometry for unchanged spatial topology."""
    ...
    outside = ndi.distance_transform_edt(~native_footprint, sampling=tuple(spacing_array)).astype(np.float32)
    fixed_shape = (int(_cfg(config, "patch_size", 48)),) * 3
    return dict(model_input=_resample_exact(ct_norm, fixed_shape, order=1).astype(np.float32)[None],
                ct_norm=ct_norm, footprint=native_footprint, organ=organ,
                liver_depth_mm=depth.astype(np.float32), outside_tumor_mm=outside)
...
def canonical_coordinate_sets(
    ...
    *,
    ...
    validate_canonical_coordinates(fields, coordinates, config, allow_empty_context=allow_empty_context)
